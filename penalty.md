A integração dessas novas camadas altera a topologia de execução de um fluxo puramente linear para uma arquitetura com bifurcação de estado e interceptação vetorial. No modelo mental de hardware e execução que você estruturou, o fluxo completo de tensores e matrizes opera como uma cadeia de dependências determinística.

O encaixe exato dessas fórmulas obedece a uma ordem física e lógica dentro dos chips, ocorrendo imediatamente após o cálculo dos estados ocultos finais e antes que o `LM Head` converta esses estados em índices discretos de tokens.

### O Pipeline Unificado de Execução
...
$$\text{Fórmula (1): } X_q = \text{round}\left( \frac{X}{\alpha} \right) \times 2^E$$
...
$$\text{Fórmula (2): } Y = \text{Accumulator}(X_q W_q^T) \times \alpha_X \alpha_W + \beta$$
...
$$\text{Fórmula (3): } m_j = \max(m_{j-1}, x_j)$$


$$\text{Fórmula (4): } l_j = l_{j-1} e^{m_{j-1} - m_j} + e^{x_j - m_j}$$


$$\text{Fórmula (5): } \text{Out}_j = \frac{l_{j-1} e^{m_{j-1} - m_j} \text{Out}_{j-1} + e^{x_j - m_j} V_j}{l_j}$$
...
$$\text{Fórmula (6): } G(x) = \text{Softmax}(W_g x)$$


$$\text{Fórmula (7): } y_i = \sum_{k=1}^K G(x_i)_k \cdot \text{Expert}_k(x_i)$$
...
$$\text{Fórmula (8): } \text{RMS}(a) = \sqrt{\frac{1}{d}\sum_{i=1}^{d} a_i^2 + \epsilon}$$


$$\text{Fórmula (9): } y = \frac{a}{\text{RMS}(a)} \odot \gamma$$
...
$$\text{Fórmula (10): } z_{CFG} = z_{uncond} + \gamma (z_{cond} - z_{uncond})$$
...
$$\text{Fórmula (11): } z_i' = z_{CFG, i} - (\theta \cdot c_i)$$
...
$$\text{Fórmula (12): } z_i' = -\infty \quad \text{se } i \in \text{Estado Violado}$$
...
$$\text{Fórmula (13): } P(y_i) = \frac{\exp(z_i' / T)}{\sum_{j=1}^V \exp(z_j' / T)}$$
