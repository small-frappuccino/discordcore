...
**Escopo Topológico:** Matriz isolada do repositório, impondo foco absoluto e exclusivo ao diretório `pkg/app`, obliterando e ignorando categoricamente toda a malha de código adjacente.

Inicie a varredura analítica-mecânica estritamente sobre as ramificações internas deste escopo. Mapeie e corrija anomalias baseando-se nos seguintes invariantes de hardware e *runtime*:
...
Instancie a malha de teste do compilador engatando a execução ininterrupta do comando `go test -race -v ./pkg/app/...` sobre os pacotes refatorados. O ciclo de tokenização e consolidação da rotina só deve ser interrompido e declarado concluído quando o motor de execução confirmar matematicamente:
