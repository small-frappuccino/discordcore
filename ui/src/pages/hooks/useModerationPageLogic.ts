...
export function useModerationPageLogic() {
  ...
  const { data: automodRes, isLoading: automodLoading } = useGuildFeatureQuery(client, selectedGuildID, "services.automod");
  const { data: settingsRes, isLoading: settingsLoading } = useGuildSettingsQuery(client, selectedGuildID);

  const automodMutation = usePatchGuildFeatureMutation(client, selectedGuildID, "services.automod");
  ...
  const isLoading = automodLoading || settingsLoading;
