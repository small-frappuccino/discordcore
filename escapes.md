context # DISCORDCORE :: AGENTIC EXECUTION HARNESS  (/goal)
...
## TASK
Run `python -c "import re; f = open('escapes.txt', 'r', encoding='utf-16'); lines = f.readlines(); p1 = re.compile(r'escapes to heap|moved to heap'); p2 = re.compile(r'pkg/(moderation/fast_parser|moderation/moderation_router|core/registry_cow|app/gateway_ingress|app/command_handler)'); p3 = re.compile(r'New[A-Z]|init\.func|\.deferwrap|copyCheck|<autogenerated>'); res = [l for l in lines if p1.search(l) and p2.search(l) and not p3.search(l)]; print(''.join(res))"`. Eliminate all instances of escapes to heap.
...
- **VERIFY (termination is external and deterministic):** the loop is DONE only when all four gates are green —
    - `go build ./...`
    - `go test -race ./...`
    - `python -c "import re; f = open('escapes.txt', 'r', encoding='utf-16'); lines = f.readlines(); p1 = re.compile(r'escapes to heap|moved to heap'); p2 = re.compile(r'pkg/(moderation/fast_parser|moderation/moderation_router|core/registry_cow|app/gateway_ingress|app/command_handler)'); p3 = re.compile(r'New[A-Z]|init\.func|\.deferwrap|copyCheck|<autogenerated>'); res = [l for l in lines if p1.search(l) and p2.search(l) and not p3.search(l)]; print(''.join(res))"` → zero new heap escapes on the touched hot path
    - `golangci-lint run`
