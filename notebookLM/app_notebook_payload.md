...
type CommandGroup interface {
	// Commands exposes the static slice of Discord Application Command blueprints generated by the vertical's internal NewCommands() constructor.
	...
	// Handlers exposes the O(1) routing dictionary binding the unique command invocation string directly to its procedural execution lane.
...
type FeatureService interface {
	...
	// OnConfigMutated is the passive Pub/Sub reactive receiver. It receives a static, read-only snapshot of the configuration.
	...
	OnConfigMutated(ctx context.Context, snapshot files.ConfigSnapshot)
}
