...
func makeRESTError(statusCode int, code int, message string) *httputil.HTTPError {
	restErr := &httputil.HTTPError{}
	if statusCode != 0 {
		restErr.Status = statusCode
	}
	if code != 0 || message != "" {
		restErr.Code = httputil.ErrorCode(code)
		restErr.Message = message
	}
	return restErr
}
...
func TestClassifyFetchError(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unknown channel code", err: makeRESTError(0, 10003, "unknown channel"), want: FailureClassMissingChannel},
		...
		{name: "missing access code", err: makeRESTError(0, 50001, "missing access"), want: FailureClassForbidden},
		{name: "missing permissions code", err: makeRESTError(0, 50013, "missing permissions"), want: FailureClassForbidden},
		...
		{name: "unknown message code unrelated to fetch", err: makeRESTError(0, 10008, "unknown message"), want: FailureClassUnknown},
		{name: "bulk delete age unrelated to fetch", err: makeRESTError(http.StatusBadRequest, 50034, "older than 14 days"), want: FailureClassUnknown},
...
func TestClassifyDeleteError(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unknown message code", err: makeRESTError(0, 10008, "unknown message"), want: FailureClassMissingMessage},
		{name: "unknown channel code", err: makeRESTError(0, 10003, "unknown channel"), want: FailureClassMissingChannel},
		{name: "403 response", err: makeRESTError(http.StatusForbidden, 0, "forbidden"), want: FailureClassForbidden},
		{name: "401 response", err: makeRESTError(http.StatusUnauthorized, 0, "unauthorized"), want: FailureClassForbidden},
		{name: "missing access code", err: makeRESTError(0, 50001, "missing access"), want: FailureClassForbidden},
		{name: "missing permissions code", err: makeRESTError(0, 50013, "missing permissions"), want: FailureClassForbidden},
		{name: "bulk delete age code", err: makeRESTError(http.StatusBadRequest, 50034, "older than 14 days"), want: FailureClassBulkDeleteAge},
		{name: "rate limited", err: makeRESTError(http.StatusTooManyRequests, 0, "rate limited"), want: FailureClassRateLimited},
		{name: "5xx response", err: makeRESTError(http.StatusInternalServerError, 0, "boom"), want: FailureClassTransient},
		{name: "wrapped 404", err: fmt.Errorf("delete failed: %w", makeRESTError(http.StatusNotFound, 0, "gone")), want: FailureClassMissingMessage},
		{name: "wrapped age", err: fmt.Errorf("bulk failed: %w", makeRESTError(0, 50034, "older than 14 days")), want: FailureClassBulkDeleteAge},
