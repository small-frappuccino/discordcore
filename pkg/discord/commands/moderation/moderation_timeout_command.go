...
// Handle handles.
func (c *timeoutCommand) Handle(ctx *legacycore.Context) error {
	if enabled, _ := ctx.Config.Config().ResolveFeatures(ctx.GuildID).Lookup("moderation.timeout"); !enabled {
		return legacycore.NewMissingConfigError(ctx.GuildID, "Moderation Timeout", "/moderation")
	}
	...
	if minutes <= 0 {
		return &legacycore.CommandError{Message: "Please provide a valid timeout duration in minutes.", Ephemeral: true}
	}
	if minutes > timeoutMaxMinutes {
		return &legacycore.CommandError{Message: "Timeout duration cannot exceed 40320 minutes (28 days).", Ephemeral: true}
	}
	...
	if ok, reasonText := canTimeoutTarget(ctx, timeoutCtx, userID); !ok {
		return &legacycore.CommandError{Message: fmt.Sprintf("Cannot timeout `%s`: %s.", userID, reasonText), Ephemeral: true}
	}
	...
	if err := ctx.Session.GuildMemberTimeout(ctx.GuildID, userID, &until, discordgo.WithAuditLogReason(reason)); err != nil {
		return &legacycore.CommandError{Message: fmt.Sprintf("Failed to timeout user %s: %v", userID, err), Ephemeral: true}
	}
	...
	return legacycore.NewResponseBuilder(ctx.Session).Success(ctx.Interaction, buildTimeoutCommandMessage(targetUsername, minutes, reason, truncated))
}
