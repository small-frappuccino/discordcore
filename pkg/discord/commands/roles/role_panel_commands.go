...
func (c *rolePanelSetSubCommand) Handle(ctx *core.Context) error {
	...
	if extractor.HasOption(rolePanelOptionTitle) {
		title = extractor.String(rolePanelOptionTitle)
	}
	if extractor.HasOption(rolePanelOptionDescription) {
		description = extractor.String(rolePanelOptionDescription)
	}
	if extractor.HasOption(rolePanelOptionColor) {
		color = int(extractor.Int(rolePanelOptionColor))
	}

	if err := c.configManager.SetRolePanelEmbed(ctx.GuildID, key, title, description, color); err != nil {
