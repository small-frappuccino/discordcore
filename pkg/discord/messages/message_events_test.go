...
func TestMessageEventService_ProcessMessageUpdateQueuesAsyncPersistence(t *testing.T) {
	...
	metrics := domainmessages.NewInMemoryMessageWriterMetrics()
	writer := domainmessages.NewMessageCreateWriter(store, metrics, slog.Default())
	...
	if got := snap.Flush.FlushedByOp[domainmessages.MessageWriterFlushOpVersions]; got < 2 {
...
func TestMessageEventService_ProcessMessageDeleteQueuesAsyncPersistenceWhenDeleteOnLogEnabled(t *testing.T) {
	...
	writer := domainmessages.NewMessageCreateWriter(store, nil, slog.Default())
	writer.flushInterval = time.Hour
	service.messageCreateWriter = writer
	writer.Start()
	defer func() {
