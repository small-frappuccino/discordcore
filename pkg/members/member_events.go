...
func NewMemberEventService(session *discordgo.Session, configManager *files.ConfigManager, notifier *notifications.NotificationSender, store *storage.Store, logger *slog.Logger) *MemberEventService {
...
func (mes *MemberEventService) Start(ctx context.Context) error {
	...
	mes.handlerCancels = append(mes.handlerCancels,
		mes.session.AddHandler(monitoring.GuardedHandler(&mes.lifecycle, mes.handleGuildMemberAdd)),
		mes.session.AddHandler(monitoring.GuardedHandler(&mes.lifecycle, mes.handleGuildMemberUpdate)),
		mes.session.AddHandler(monitoring.GuardedHandler(&mes.lifecycle, mes.handleGuildMemberRemove)),
	)
...
func (mes *MemberEventService) getGuildMember(ctx context.Context, guildID, userID string) (*discordgo.Member, error) {
	...
	return monitoring.RunWithTimeout(ctx, monitoring.DependencyTimeout, func() (*discordgo.Member, error) {
