...
func (s *StatsService) UpdateStatsChannels(ctx context.Context) error {
	if s == nil || s.publisher == nil || s.configManager == nil {
		return nil
	}
	cfg := s.scopedConfig()
...
func statsSnapshotFromMember(member GuildMember, trackedRoles map[string]struct{}) (string, statsMemberSnapshot, bool) {
...
func (s *StatsService) updateStatsChannelName(ctx context.Context, guildID string, cfg files.StatsChannelConfig, count int) error {
	...
	if err := s.publisher.ChannelEditName(ctx, channelID, newName); err != nil {
...
func (s *StatsService) resolveChannel(ctx context.Context, channelID string) (Channel, error) {
	return s.publisher.Channel(ctx, channelID)
}
...
// ApplyStatsMemberAdd applys stats member add.
func (s *StatsService) ApplyStatsMemberAdd(guildID, userID string, isBot bool, roles []string, joinedAt time.Time) {
...
func (s *StatsService) streamGuildMembers(ctx context.Context, guildID string) iter.Seq2[GuildMember, error] {
	if s == nil || s.publisher == nil {
		return func(yield func(GuildMember, error) bool) {
			yield(GuildMember{}, fmt.Errorf("publisher is unavailable"))
		}
	}
	return s.publisher.StreamGuildMembers(ctx, guildID)
}
